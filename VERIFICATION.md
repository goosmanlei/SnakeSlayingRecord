...
| 标准 | 当前证据与缺口 |
| --- | --- |
| 1. 指定资料清理 | 本机运行库按精确 ID 移除通行本、四库本、陈峰外链三项资料及相应对象修订，1 条专属评论与 5 条评论事件；移除共用 SVG，并更新白话译注元数据、PROJECT 当前配置及旧事件中的直接引用。译文五段与词语说明正文保持。Schema 3 导出为 `sources=1, comments=0, events=0, objects=1, revisions=1`，清单不含素材；主实例 `/api/sources` 只返回白话译注，`/api/comments/context` 为空，旧 SVG 路径为 404。远端与干净恢复证据见下文同步核验后追加。 |
...
| 8. 完整审阅与同步 | 清理后正式实例浏览器只见白话译注且无旧图片；正式媒体播放和新内容评论无法验收。系统仓库 9 项单元测试、JS 语法和 diff 检查通过；导出/推送/干净恢复需另行核验。 |
